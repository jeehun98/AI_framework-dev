...
def test_activation_regression(act_name, epochs=600, lr=1e-2):
    ...
    model.add(Activation(act_name))             # 새 활성화 사용
...
def test_softmax_mse(epochs=2000, lr=1e-2):
    ...
    # 경사 흐름 확인용 기준 (완화)
    assert_loss_decreases("softmax/3class", loss_before, loss_after, factor=0.82)

    # 러프 정확도 확인
    y_pred = model.predict(X)
    acc = (np.argmax(y_pred, axis=1) == np.argmax(Y, axis=1)).mean()
    print(f"[softmax] rough acc: {acc*100:.1f}%")

def test_softmax_cce(epochs=1200, lr=3e-3):
    X, Y = make_simple_3class(seed=123, n_per_class=64, noise=0.15)
    model = Sequential(input_shape=(1,1,3))
    model.add(Flatten(input_shape=(1,1,3)))
...
def main():
    ...
    test_softmax_cce(epochs=2000, lr=1e-2)
