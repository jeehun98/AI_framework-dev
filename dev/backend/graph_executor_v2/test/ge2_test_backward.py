...
def run_case(M=64, N=48, K=32,
    ...
    ge2.gemm_bias_act_bwd_f32_ex(
        ...
        int(C.data.ptr) if C is not None else None,
        int(gY.data.ptr),
        int(Z.data.ptr) if Z is not None else int(cp.asarray(hPre).data.ptr),  # 만약 Z 저장 안했다면 CPU pre로 대체(검증용)
    ...
    # --- CPU ref backward
    hZ  = hPre  # forward에서 계산된 pre-activation
    hGZ = hGY * dact(hZ, act_kind, leaky)

    hGA = np.empty((M,K), np.float32)
    for m in range(M):
        for k in range(K):
            hGA[m,k] = np.dot(hGZ[m,:], hB[k,:])
    hGB = np.empty((K,N), np.float32)
    for k in range(K):
        for n in range(N):
            hGB[k,n] = np.dot(hA[:,k], hGZ[:,n])
    ...
    tol = 3e-4
    ok = (fwd_diff<tol and dA_diff<tol and dB_diff<tol and (not use_C or dC_diff<tol) and (gBias is None or dBias_diff<tol))
    print(f"[RESULT] OK={ok} (tol={tol:.1e})")
    return ok

if __name__ == "__main__":
    # 대표 케이스 1: GELU + PerN bias + C 사용 + Z stash
    run_case(M=64, N=48, K=32,
             act_kind=ge2.ActKind.GELU,
             bias_kind=ge2.BiasKind.PerN,
             use_C=True, save_preact=True)

    # 대표 케이스 2: ReLU + Scalar bias + C 미사용 + Z stash
    run_case(M=64, N=64, K=32,
             act_kind=ge2.ActKind.ReLU,
             bias_kind=ge2.BiasKind.Scalar,
             use_C=False, save_preact=True)

    # 대표 케이스 3: LeakyReLU + PerM bias + C 사용 + Z stash
