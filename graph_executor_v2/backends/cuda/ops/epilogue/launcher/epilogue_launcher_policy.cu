...
Status run(const Plan& plan, const Tensors& ts,
  ...
  // === 예시 1: FP32 ReLU + PerN-Bias (+/- Dropout) ===
      ...
      plan.attrs.act==ActKind::ReLU && plan.attrs.bias==BiasKind::PerN)
  {
    ...
    if (plan.attrs.dropout) {
      using Policy = EpPolicy<float, /*bias*/true, /*ReLU*/1, /*drop*/true, /*resid*/false>;
      ep_kernel_policy<Policy, EpParamsF32><<<grid,block,0,s>>>(p);
    } else {
      using Policy = EpPolicy<float, /*bias*/true, /*ReLU*/1, /*drop*/false, /*resid*/false>;
      ep_kernel_policy<Policy, EpParamsF32><<<grid,block,0,s>>>(p);
    }
    return {true,""};
  }

  // === 예시 2: FP16 GELU + no-bias + Dropout ===
  if (xdt==DType::F16 && ydt==DType::F16 &&
      plan.attrs.act==ActKind::GELU && plan.attrs.bias==BiasKind::None &&
      plan.attrs.dropout)
  {
    auto p = pack_f16(plan, ts);
    using Policy = EpPolicy<half, /*bias*/false, /*GELU*/2, /*drop*/true, /*resid*/false>;
