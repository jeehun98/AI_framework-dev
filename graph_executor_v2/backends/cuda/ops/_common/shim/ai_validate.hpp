...
#include "ai_status.hpp"       // Status, AI_RETURN_IF_ERROR
#include "ai_tensor.hpp"       // Tensor, DType, Layout, TensorDesc 등 (shim)
...
// -------- 기초 체크 --------
inline Status expect_cuda(const Tensor& T) {
  return T.is_cuda() ? Status::Ok : Status::DeviceMismatch;
}
inline Status expect_layout(const Tensor& T, Layout L) {
  return (T.desc.layout == L) ? Status::Ok : Status::LayoutMismatch;
}
inline Status expect_rank(const Tensor& T, int k) {
  return static_cast<int>(T.desc.shape.size()) == k ? Status::Ok : Status::ShapeMismatch;
}
...
inline Status expect_positive_numel(const Tensor& T) {
  return (T.numel() > 0) ? Status::Ok : Status::ShapeMismatch;
}
...
// 흔한 단축형
inline Status expect_rowmajor_4d(const Tensor& T) {
  return expect_rowmajor_nd(T, 4, {DType::F16, DType::F32, DType::BF16});
}
inline Status expect_rowmajor_1d(const Tensor& T) {
  return expect_rowmajor_nd(T, 1, {DType::F16, DType::F32, DType::BF16});
}
...
// -------- dtype 조합 --------
inline Status expect_io_mixed_f16_f32(const Tensor& T) {
  return expect_dtype_in(T, {DType::F16, DType::F32});
}
inline Status expect_param_f32(const Tensor& T) {
  return (T.desc.dtype == DType::F32) ? Status::Ok : Status::DtypeMismatch;
}
...
inline Status get_dims_4d(const Tensor& X, bool channels_last, NCHW4& out) {
  ...
  out = channels_last ? NCHW4{static_cast<int>(s[0]), static_cast<int>(s[3]),
                              static_cast<int>(s[1]), static_cast<int>(s[2])}
                      : NCHW4{static_cast<int>(s[0]), static_cast<int>(s[1]),
...
inline Status expect_same_except_axis(const std::vector<Tensor>& Xs, int rank, int axis_norm) {
  ...
  const auto& ref = Xs[0];
  AI_RETURN_IF_ERROR(expect_rowmajor_nd(ref, rank, {ref.desc.dtype})); // dtype 동일성만 확인

  for (size_t i = 1; i < Xs.size(); ++i) {
    const auto& t = Xs[i];
    if (t.desc.layout != ref.desc.layout) return Status::LayoutMismatch;
    if (static_cast<int>(t.desc.shape.size()) != rank) return Status::ShapeMismatch;
...
// -------- 자주 쓰는 특이 케이스 --------
[[nodiscard]] inline bool is_cuda_f32_rowmajor(const Tensor& T) noexcept {
  return T.is_cuda()
