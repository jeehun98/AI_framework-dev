...
struct Range {
  explicit Range(const char* name)
    : Range(name, static_cast<std::uint32_t>(Color::Gray)) {}

  Range(const char* name, std::uint32_t argb_color) {
...
#define AI_NVTX_RANGE(name, color_enum) \
  ::ai::cuda::shim::nvtx::Range AI_NVTX_UNIQUE(_nvtx_range_) { (name), static_cast<std::uint32_t>(color_enum) }

// 기본색(Gray) 범위
#define AI_NVTX_RANGE_GRAY(name) \
  ::ai::cuda::shim::nvtx::Range AI_NVTX_UNIQUE(_nvtx_range_) { (name) }

// 포인트 마킹
#define AI_NVTX_MARK(name) ::ai::cuda::shim::nvtx::mark(name)

// ==== 과거 호환 별칭 ====
// 예전 코드: NVTX_RANGE("tag", ai::cuda::shim::nvtx::Color::Blue);
#ifndef NVTX_RANGE
