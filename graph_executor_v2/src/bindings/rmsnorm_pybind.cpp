...
PYBIND11_MODULE(_ops_rmsnorm, m) {
  m.doc() = "Independent CUDA RMSNorm op bindings (forward/backward)";
  ...
  m.def("forward",
    [](uintptr_t x_ptr, const std::vector<int64_t>& x_shape,
       ...
       uintptr_t stream_ptr) {
        ...
        auto st = RMSNormCudaLaunch(X, gamma, beta, Y, attrs, s);
  ...
  m.def("backward",
    [](uintptr_t x_ptr,  const std::vector<int64_t>& x_shape,
       ...
       uintptr_t stream_ptr) {
        ...
        StreamHandle s = reinterpret_cast<StreamHandle>(stream_ptr);
        auto st = RMSNormCudaBackwardLaunch(X, gamma, dY, dX, dgamma, dbeta, attrs, s);
