...
# 요구:
#  - Conv2D/Dense가 forward_into/backward_into 지원
...
def make_model(
    ...
    *,
    ...
    """
    간단 CNN:
      [Conv2D(f1, k=3, pad=1, ReLU)] → [Conv2D(f2, 3x3, pad=1, ReLU)](선택) → Flatten → Dense(hidden, ReLU) → Dense(C)
    """
    layers = [
...
def main():
    ...
    # Eager 비교를 위해 동일 모델 새로 생성
    model_eager = make_model(N, Cin, H, W, f1=args.f1, f2=args.f2,
                             hidden=args.hidden, classes=C, out_scale=args.out_scale)
