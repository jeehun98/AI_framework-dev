...
def test_graph_key_preview_matches_real(tag="key_preview"):
    ...
    if n_preview is not None and n_real is not None:
        ...
        # variant: 필수 키만 부분집합 비교 (path_fp는 preview가 비어있으면 관용 처리)
        ...
        for k in must_keys:
            if k not in pvd:
                # preview에 없으면 스킵(빌드마다 추가/생략 가능)
                continue
            pv = pvd[k]
            rv = rvd.get(k, None)
            if k == "path_fp":
                # preview가 비어있으면((), None) — 동적 경로 미평탄화 빌드 허용
                if pv in ((), None):
                    # 관용: 비교 패스
                    continue
                # 둘 다 비어있지 않다면 정확히 일치해야 함
...
def test_ir_dump_minimal(tag="ir_dump"):
    ...
    ir = getattr(model, "last_graph_ir", None) \
         or getattr(model, "_last_graph_ir", None)
