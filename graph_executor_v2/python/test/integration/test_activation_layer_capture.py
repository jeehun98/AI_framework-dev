# test/integration/test_activation_layer_trainer_smoke.py
import os, sys
...
def make_model_with_act(act: str, N=8, Cin=3, H=16, W=16, hidden=32, classes=5):
    """
    Conv2D/Dense는 activation="none"으로 두고, 별도의 ActivationLayer를 사이에 삽입.
    """
    # leaky/elu 파라미터는 기본값 사용(neg_slope=0.01, alpha=1.0)
...
def run_smoke_for_activation(act: str):
    ...
    for t in range(3):
        ...
        print(f"[SMOKE][{act}] step={t:02d} loss={L:.6f}")
        assert isinstance(L, float) and math.isfinite(L)
        last_L = L

    print(f"[OK] Integrated trainer smoke passed with ActivationLayer({act}). Last loss={last_L:.6f}")


def main():
    print("== Integrated trainer smoke with ActivationLayer inserted ==")
    # 모든 지원 activation 순회
    acts = ["none", "relu", "leaky_relu", "gelu", "silu", "sigmoid", "tanh", "elu", "softplus"]
    for a in acts:
        run_smoke_for_activation(a)
