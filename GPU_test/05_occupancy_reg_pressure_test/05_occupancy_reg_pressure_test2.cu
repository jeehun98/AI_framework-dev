// occupancy_reg_pressure_test_v2.cu
                                 ...
                                 int iters)
{
    ...
    // iters 회 동안 모든 애들에 일을 시켜서 레지스터 살아있게 유지
    for (int i = 0; i < iters; ++i) {
...
int main()
{
    ...
    const int num_blocks = 4096;        // 총 threads = 256 * 4096 = 1,048,576
    const int num_elems  = block_size * num_blocks;

    const int iters_low  = 2000;
    // high는 레지스터 96개 정도 × iters_high → 대충 비슷하게 맞추고 싶으면
    const int iters_high = 32;          // 96 * 32 ≈ 3072, 살짝 더 많게
