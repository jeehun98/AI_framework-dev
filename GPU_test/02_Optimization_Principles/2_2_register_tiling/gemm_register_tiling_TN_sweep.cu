...
// 타일 크기 설정
constexpr int BM = 16;   // block이 담당하는 M 방향 타일
constexpr int BK = 16;   // K 타일 두께
// BN = 16 * TN (템플릿에서 결정)

// 스레드 블록 크기 (고정)
constexpr int TX = 16;   // threadIdx.x
constexpr int TY = 16;   // threadIdx.y
...
// =======================
//
// - 블록이 담당하는 C 영역: BM x BN
//   * BM = 16, BN = 16 * TN
// - blockDim = (TX=16, TY=16)
//   * threadIdx.y: M 방향 1개 row 담당
//   * threadIdx.x: N 방향 1개 "thread column" 담당
//   * 각 thread가 N 방향으로 TN개 결과를 레지스터에 가짐: acc[TN]
//
// - shared memory:
//   * As[BM][BK]      : A 타일 (16x16)
//   * Bs[BK][BN]      : B 타일 (16 x (16*TN))
//
template<int TN>
    ...
    int M, int N, int K)
{
    ...
    static_assert(BM == 16, "BM must be 16 for this kernel");
    static_assert(BK == 16, "BK must be 16 for this kernel");

    // block이 담당하는 C 타일 시작 좌표
    int block_row = blockIdx.y * BM;  // M 방향
    int block_col = blockIdx.x * BN;  // N 방향

    int ty = threadIdx.y; // [0, TY)
    int tx = threadIdx.x; // [0, TX)

    // 이 thread가 담당하는 C의 행/열 시작 위치
    int row = block_row + ty;
    int col_base = block_col + tx;  // 여기서부터 TN만큼 stride = TX

    // shared tiles
    __shared__ float As[BM][BK];    // [16][16]
    __shared__ float Bs[BK][BN];    // [16][16*TN]
    ...
    for (int k0 = 0; k0 < K; k0 += BK) {
        ...
        if (row < M) {
            ...
            for (int kk = 0; kk < BK; ++kk) {
                float a_val = As[ty][kk];  // 이 thread의 row에 해당하는 A 값

                // 이 thread가 담당하는 TN개의 column에 대해 B 값을 읽음
                #pragma unroll
                for (int t = 0; t < TN; ++t) {
                    int col = col_base + t * TX;
                    float b_val = 0.0f;
                    if (col < N) {
                        int b_col_in_tile = tx + t * TX; // [0, BN)
                ...
                const float* hC_ref, float* hC_tmp)
{
    ...
    printf("Config %-10s  TN=%d  BN=%3d  |  smem=%.1f KB  |  time=%.3f ms  |  max diff=%e\n",
...
int main()
{
    printf("=== Register Tiling Test: TN sweep (1,2,4,8) ===\n");
...
/*
nvcc -O3 -arch=sm_86 -lineinfo -o gemm_register_tiling_TN_sweep.exe gemm_register_tiling_TN_sweep.cu

ncu --kernel-name regex:gemm_reg_tiling_TN_kernel<1>.* --metrics smsp__inst_executed.sum,smsp__pipe_fma_cycles_active.avg.pct_of_peak_sustained_elapsed ./gemm_register_tiling_TN_sweep.exe
