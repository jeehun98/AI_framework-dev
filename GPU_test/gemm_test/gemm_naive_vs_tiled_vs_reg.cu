    ...
    int iters)
{
    ...
    // warmup
    for (int i = 0; i < 3; ++i) {
        gemm_naive_kernel<float><<<grid, block>>>(
            M, N, K, dA, dB, dC);
    }
...
// ============================================================================
// main: naive vs tiled vs tiled+reg 비교
// ============================================================================
...
int main()
{
    ...
    printf("Compare naive vs tiled vs tiled+reg GEMM\n");
    ...
    printf("\nSpeed ratios (time):\n");
    printf("  tiled     / naive = %.4fx\n", ms_tiled / ms_naive);
    printf("  tiled+reg / naive = %.4fx\n", ms_reg   / ms_naive);
    printf("  tiled+reg / tiled = %.4fx\n", ms_reg   / ms_tiled);

    printf("\nDiff checks:\n");
    printf("  naive vs tiled:\n");
    check_max_diff(hC_naive, hC_tiled, M, N);
    printf("  naive vs tiled+reg:\n");
...
// nvcc -O3 -arch=sm_86 gemm_naive_vs_tiled_vs_reg.cu -o gemm_gemm_test
